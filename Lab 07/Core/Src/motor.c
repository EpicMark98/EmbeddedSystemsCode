...
void encoder_init(void) {
    ...
    // Set up encoder input pins (TIMER 3 CH1 and CH2) (B4 and B5 encoder input?)
    ...
    // Select PSC and ARR values that give an appropriate interrupt rate (45 ms)
    TIM6->PSC = 11;
    TIM6->ARR = 30000;
...
void ADC_init(void) {
    ...
    // Configure PA1 (curr sense?) for ADC input (used for current monitoring)
...
void PI_update(void) {
     ...
     *
     * target_rpm -> target motor speed in RPM															-	input provided by main loop
     * motor_speed -> raw motor speed in encoder counts											- input provided by interrupt (in ticks/45 ms) (64 ticks per motor shaft revolution)
     * error -> error signal (difference between measured speed and target)
     * error_integral -> integrated error signal
     * Kp -> Proportional Gain																							- constant?
     * Ki -> Integral Gain																									- constant?
     * output -> raw output signal from PI controller												- output from this function
     * duty_cycle -> used to report the duty cycle of the system 						- debug value set by this function
     * adc_value -> raw ADC counts to report current												- debug value set by this function
     *
     */
    ...
    /// TODO: Calculate integral portion of PI controller, write to "error_integral" variable
		error_integral = error_integral_prev + Ki*error;
